...
{ 
    public interface ILibraryDal
    {
        ...
        List<Libraries> SearchBooksByName(string key);
        List<Libraries> SearchBooksByAuthor(string key);
        List<Libraries> SearchBooksByCategory(string key);
        ...
        int GetTotalOfReadPages();
        List<Libraries> GetCurrentlyReadingData();
    }
}
